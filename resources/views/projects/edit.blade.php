...
<div class="px-6 lg:px-8 my-24">
    ...
    <h1 class="text-4xl font-bold text-gray-800 dark:text-white mb-8">Edit {{ $project->project_name }} Project</h1>
        ...
        <div class="flex flex-row w-full flex-wrap gap-5">
            <div class="grow" id="first-half">
                <div class="mt-4 flex flex-col gap-1">
                    ...
                    <input type="text" name="project_name" class="px-3 py-3 rounded-md bg-gray-100" placeholder="Enter project name here" value="{{ old('project_name', $project->project_name) }}">
                </div>
                <div class="mt-4 flex flex-col gap-1">
                    <label for="description" class="font-semibold text-lg text-gray-700 dark:text-gray-100">Description:</label>
                    <textarea type="text" name="description" class="px-3 py-3 rounded-md bg-gray-100" placeholder="Enter project description here (optional)">{{ old('description', $project->description) }}</textarea>
                </div>
                <div class="mt-4 flex flex-col md:flex-row flex-wrap gap-4">
                        ...
                        <select type="number" name="employee_id" class="px-3 py-3 rounded-md bg-gray-100">
                            @if (isset($employees) && $employees->isNotEmpty())
                                ...
                                @foreach ($employees as $employee)
                                    <option value="{{ $employee->id }}" {{ $project->employees->contains('id', $employee->id) ? 'selected' : '' }}>{{ $employee->nama_lengkap }}</option>
                                @endforeach
                            @else
                    ...
                    <div class="flex flex-col gap-1 grow">
                        ...
                        <input type="text" name="role" class="px-3 py-3 rounded-md bg-gray-100" placeholder="Enter initial role here" value="{{ old('role', optional($project->employees->first()->pivot)->role) }}">
                    </div>
                </div>
            </div>
            <div class="grow">
                <div class="mt-4 flex flex-col gap-1">
                    ...
                    <input type="date" name="start_date" class="px-3 py-3 rounded-md bg-gray-100" value="{{ old('start_date', $project->start_date) }}">
                </div>
                <div class="mt-4 flex flex-col gap-1">
                    <label for="finish_date" class="font-semibold text-lg text-gray-700 dark:text-gray-100">Finish Date (optional):</label>
                    <input type="date" name="finish_date" class="px-3 py-3 rounded-md bg-gray-100" value="{{ old('finish_date', $project->finish_date) }}">
                </div>
                <div class="mt-4 flex flex-col gap-1">
                    ...
                    <select name="status" class="px-3 py-3 rounded-md bg-gray-100">
                        <option value="planned" {{ $project->status == "planned" ? 'selected' : '' }}><i class="bi bi-calendar-week text-indigo-600"></i>&nbsp; Planned</option>
                        <option value="ongoing" {{ $project->status == "ongoing" ? 'selected' : '' }}><i class="bi bi-clock-history text-indigo-600"></i>&nbsp; Ongoing</option>
                        <option value="finished" {{ $project->status == "finished" ? 'selected' : '' }}><i class="bi bi-check2-circle text-indigo-600"></i>&nbsp; Finished</option>
                    </select>
                </div>
