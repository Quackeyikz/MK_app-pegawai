...
<div class="px-6 lg:px-8 my-24 flex flex-col justify-center items-center">
    ...
    <h1 class="text-4xl font-bold text-gray-800 dark:text-white mb-8">Create New Position</h1>

    <form action="{{ route('positions.store') }}" method="POST" class="w-full md:w-[90%] lg:w-[70%]">
        @csrf
        <div class="flex flex-col flex-wrap gap-5">
            <div class="grow">
                <label for="nama_jabatan" class="font-semibold text-lg text-gray-700 dark:text-gray-100">Position Name:</label>
                <input type="text" name="nama_jabatan" class="px-3 py-3 mt-3 rounded-md bg-gray-100 w-full" placeholder="Enter new position name">
            </div>
            <div class="grow">
                <label for="gaji_pokok" class="font-semibold text-lg text-gray-700 dark:text-gray-100">Main Salary:</label>
                <input type="number" name="gaji_pokok" class="px-3 py-3 mt-3 rounded-md bg-gray-100 w-full" placeholder="Enter salary amount here">
            </div>
