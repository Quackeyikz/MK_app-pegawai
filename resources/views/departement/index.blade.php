...
@section('content')
    ...
    <div class="px-6 lg:px-8 mt-24">
        ...
        <h1 class="text-4xl text-center font-bold text-gray-800 dark:text-white mb-8">Department List</h1>
                    ...
                    @foreach ($departments as $department)
                        <tr class="hover:bg-indigo-300/20 dark:hover:bg-indigo-400/50">
                            ...
                            <td class="flex flex-row flex-nowrap gap-2">
                                <a href="{{ route('departments.edit', $department->id) }}" class="py-1 px-2 rounded-sm bg-indigo-500 hover:bg-indigo-600 text-white"><i class="bi bi-pencil-square"></i></a>
                                <form action="{{ route('departments.destroy', $department->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" onclick="return confirm('Are you sure you want to delete this department?')" class="py-1 px-2 rounded-sm hover:cursor-pointer bg-red-500 hover:bg-red-600 text-white"><i class="bi bi-trash3"></i></button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
