...
<div class="px-6 lg:px-8 my-24 flex flex-col justify-center items-center">
    ...
    <h1 class="text-4xl font-bold text-gray-800 dark:text-white mb-8">Update Department</h1>

    <form action="{{ route('departments.update', $department->id) }}" method="POST" class="w-full md:w-[90%] lg:w-[70%]">
        @csrf
        @method('PUT')
        <div class="flex flex-col flex-wrap gap-5">
            <div class="grow">
                <label for="nama_departemen" class="font-semibold text-lg text-gray-700 dark:text-gray-100">Department Name:</label>
                <input type="text" name="nama_departemen" class="px-3 py-3 mt-3 rounded-md bg-gray-100 w-full" placeholder="Enter new department name here" value="{{ old('nama_departemen', $department->nama_departemen) }}">
            </div>
